...
class Capture {
    ...
    createTCPStream(key, options) {
        ...
        this.info(`TCP stream ${chalk.bold(streamId)} ${chalk.green('CLOSED')} (${streamDescription})`);
        ...
        stream
            ...
            .on('end', () => {
                this.info(`TCP stream ${chalk.bold(streamId)} ${chalk.yellow('CLOSED')} (${streamDescription})`);
                session.close();
